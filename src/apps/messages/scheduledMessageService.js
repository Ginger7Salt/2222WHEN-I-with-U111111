...
/**
 * 生成一条"角色主动联系"消息、写入 db.messages、更新会话，
 * 并按需触发本地系统通知。
 *
 * 被两处调用方共用：
 * 1. 到期预约消息（executeScheduledMessage）
 * 2. 定时工作流（workflowScheduler）
 *
 * metadataExtra 会被合并进消息的 metadata 字段，
 * 用来标记消息来源（比如 scheduledMessageId 或 workflowId）。
 */
export const generateAndDeliverProactiveMessage = async ({
  chatId,
  characterId,
  intent = '',
  metadataExtra = {}
  ...
  const [chat, character, recentMessages] = await Promise.all([
    ...
    getRecentMessagesForChat(chatId, CONTEXT_MESSAGE_LIMIT)
  ]);
    ...
    try {
      ...
      await registration.showNotification(
        character.name || '消息提醒',
        {
          ...
          tag: `proactive-${chat.id}-${messageId}`,
          icon: '/icon-192.png'
        }
      );
    } catch (error) {
      console.warn(
        '[ProactiveMessage] 显示本地通知失败：',
        error
      );
    }
  }
